...
- 描述：向学校邮箱发送注册验证码
- 请求体（JSON）：
  - email string 必填，学校邮箱地址，格式为 URI email
  - studentId string 可选，学号/业务标识，用于绑定账号
- 返回：
  - 成功：code=0，data：{"expiresIn":300}
  - 失败：400 参数校验失败，409 邮箱已绑定/发送频率过高，500 服务器错误

2) 用户注册
- URL：POST /api/v1/auth/register
- 描述：校验邮箱验证码后完成注册
- 请求体（JSON）：
  - email string 必填，学校邮箱地址
