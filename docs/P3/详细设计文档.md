...
- `Notification`：记录站内消息，支持已读/未读状态流转。
- `RecommendationItem`：承载推荐结果（分数、排名、推荐理由标签）。
- 应用服务：负责编排流程、事务边界、跨聚合调用，不承载复杂领域规则。
- 仓储接口：只定义持久化契约，避免业务层直接依赖数据库实现。
- `RecommendationApplicationService`：聚合规则引擎输出，分页返回推荐列表。
- 后台服务：`AdminDemandReviewService`、`AdminUserManageService`、`AdminStatsService` 分别负责审核、用户治理与统计看板。
...
说明：本轮迭代的“简单推荐”和“管理后台”不新增独立核心业务表，主要复用现有表并通过字段扩展支持：
